...
struct Json_Writer
{
	...
	// 0: array
	// 1: object
	uint32_t nested_context_is_object[JSON_MAX_NEST];
	int nest_depth;
...
void json_push_context(Json_Writer *writer, Json_Context context)
{
	if (writer->nest_depth > 0) {
		...
		uint32_t *field = writer->nested_context_is_object + nested_count / 32;
		uint32_t bit = 1 << (nested_count % 32);
...
void json_pop_context(Json_Writer *writer)
{
	...
	if (nested_count < 0) {
		...
		uint32_t *cell = writer->nested_context_is_object + nested_count / 32;
		uint32_t bit = 1 << (nested_count % 32);
...
inline void json_keyl(Json_Writer *writer, const char *key, size_t length)
{
	...
	if (!json_write_quoted_string(writer, key, length)) {
		writer->fail = true;
		return;
	}
...
{
	json_keyl(writer, key, strlen(key));
}
...
inline void json_stringl(Json_Writer *writer, const char *str, size_t length)
{
	if (!json_value_prefix(writer))
		return;

	if (!json_needs_bytes(writer, 2 + length))
		return;

	if (!json_write_quoted_string(writer, str, length)) {
		writer->fail = true;
		return;
	}
}

inline void json_string(Json_Writer *writer, const char *str)
{
	json_stringl(writer, str, strlen(str));
}

inline void json_int(Json_Writer *writer, int value)
{
	if (!json_value_prefix(writer))
		return;

	if (writer->end - writer->ptr > 16) {
		writer->ptr += sprintf(writer->ptr, "%d", value);
	} else {
		char buf[16];
		int len = sprintf(buf, "%d", value);
		if (!json_needs_bytes(writer, len))
			return;
...
inline void json_key_string(Json_Writer *writer, const char *key, const char *string)
{
	json_key(writer, key);
	json_string(writer, string);
}

inline void json_key_int(Json_Writer *writer, const char *key, int value)
{
	json_key(writer, key);
	json_int(writer, value);
}
...
bool format_json_output_newline(Json_Formatter *formatter)
{
	return format_json_output(formatter,
			formatter->settings.newline,
			strlen(formatter->settings.newline));
}
